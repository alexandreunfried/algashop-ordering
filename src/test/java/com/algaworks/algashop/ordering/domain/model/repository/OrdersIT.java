...
class OrdersIT {
	...
	void shouldListExistingOrdersByYear() {
		...
		ordersPlacedByCustomerInYear = orders.placedByCustomerInYear(new CustomerId(), Year.now());
		assertThat(ordersPlacedByCustomerInYear).isEmpty();
	...
	void shouldReturnSalesQuantityByCustomer() {
		...
		assertThat(orders.salesQuantityByCustomerInYear(customerId, Year.now())).isEqualTo(2L);
